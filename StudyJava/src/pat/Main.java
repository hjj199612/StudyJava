...
public class Main extends Thread {
    ...
    public static int maxWeightCapacity(int[] weights, int maxCapacity, int i, int[][] arr) {
        ...
        if (weights[i] > maxCapacity) {//要不了
            ...
            arr[maxCapacity][i] = (arr[maxCapacity - weights[i]][i + 1] + weights[i]) > arr[maxCapacity][i + 1] ?
                    (arr[maxCapacity - weights[i]][i + 1] + weights[i]) :
                    arr[maxCapacity][i + 1];
            return arr[maxCapacity][i];
        }
    }
    ...
    static {
        codeMap.put("15", "GBK");
        codeMap.put("8", "UTF_16BE");
        codeMap.put("0", "ISO_8859_1");
    }
     ...
     * 参数 str:内容 dc：编码格式
     */
    public static String strToGbkCode(String str, String dc) {
        try {
            String code = getCharEncoder(dc);
            return parseByte2HexStr(str.getBytes(code));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
     ...
     * 根据指定编码返回字符串
     */
    public static String gbkCodeToStr(String strCode, String dc) {
        try {
            String code = getCharEncoder(dc);
            if (!(strCode == null || code == null)) {
                return new String(parseHexStr2Byte(strCode), code);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * 获取Http请求编码格式
     */
    public static String getCharEncoder(String dc) {
        return codeMap.get(dc);
    }
}